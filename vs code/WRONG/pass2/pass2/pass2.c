#include <stdio.h>
#include <stdlib.h>  // Include this header for the sprintf function
#include <string.h>
...
void main()
{
    ...
    while (strcmp(opcode, "END") != 0)
    {
        if (strcmp(opcode, "BYTE") == 0)
        {
            ...
            {
                sprintf(ad, "%X", operand[i]);  // Use sprintf instead of itoa
                fprintf(fp1, "%s", ad);
                fprintf(fp4, "%s", ad);
            }
            fprintf(fp1, "\n");
        ...
        {
            len = strlen(operand);
            sprintf(a, "%d", atoi(operand));  // Use sprintf instead of itoa
